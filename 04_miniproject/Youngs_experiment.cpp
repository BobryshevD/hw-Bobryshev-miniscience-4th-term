...
class DirichletBoundary : public SubDomain
{
  bool inside(const Array<double> &x, bool on_boundary) const
  {
    double c = 1.0 / 50;
    double x_0 = 0.5;
    return (x[0] < c - 1) || ((x[0] > -c) && (x[0] < c) && ((x[1] > 3*c+0.6) || (x[1] < -3*c-0.6) || ((x[1] > 3*c-0.6) && (x[1] < -3*c+0.6))))
     || ((x[0] > -c-0.5) && (x[0] < c-0.5) && ((x[1] > 3*c) || (x[1] < -3*c)));
  }
};
...
int main()
{
  ...
  //  L.g = g;
    ...
    *I_pr = *I_all;
    if (i%5 == 0)
    {
      std::string fileName = "snapshots_Yong's_experiment_I/wave_equation-" + std::to_string(i/5) + ".pvd"; //строит суммы квадратов напряжённостей поля
      File file(fileName);
      file << *I_all;
      std::string fileName1 = "snapshots_Yong's_experiment/wave_equation-" + std::to_string(i/5) + ".pvd";
  ...
  std::string fileName2 = "snapshots_Yong's_experiment/I.pvd";
