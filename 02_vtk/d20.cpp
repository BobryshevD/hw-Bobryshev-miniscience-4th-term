...
public:
    ...
    void move_scalar_field(double tau)
    {
        scalar_field = sin(z/15 + tau/6);
    }
};
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double scalar_field = sin(pointX/15);
            nodes[i] = CalcNode(pointX, pointY, pointZ, scalar_field, 0.0, 0.0, 0.0);
        }
    ...
    void snapshot(unsigned int snap_number) {
        ...
        scalar_field->SetName("scalar_field");
        ...
        string fileName = "snapshots/d20-step-" + std::to_string(snap_number) + ".vtu";
